...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const claimed = await claimSilentAssembly("episode", episodeId);
  if (!claimed) {
    return NextResponse.json({ error: "A silent assembly is already in progress for this episode." }, { status: 409 });
  }
  ...
  try {
    ...
    await releaseSilentAssembly("episode", episodeId);
  }
}
