...
import { generateSceneSfx, claimSfxGeneration, releaseSfxGeneration } from "@/lib/scene-audio";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const claimed = await claimSfxGeneration(sceneId);
  if (!claimed) {
    return NextResponse.json({ error: "SFX is already generating for this scene." }, { status: 409 });
  }
  ...
  try {
    ...
    await releaseSfxGeneration(sceneId);
  }
}
