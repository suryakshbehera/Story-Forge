...
### Phase 2 — Generation feedback & trust (2–3 weeks; partly backend-blocked) — 2.1–2.3 shipped 2026-09-11

- [x] 2.1 Generalised the shot-image job-claim pattern (claim + 409 + poll + stale rule) to narration/dialogue/video/music/sfx/silent/final assembly — 9 new nullable `DateTime?` claim fields (Scene ×4, DialogueLine, Story ×2, Episode ×2), per-job-type stale thresholds in `lib/generation-claims.ts`. Did **not** wait on roadmap #10 (`workers/`) — confirmed it's still an empty, unbuilt stub, and this reuses the same lightweight lock mechanism already proven for shots, no queue needed.
- [x] 2.2 Persistent job tray in `site-header.tsx` (`components/job-tray.tsx`) — polls `GET /api/projects/[id]/jobs` every 5s while on a project route, shows stage + live elapsed time, links to where the job is running. **Known v1 gap**: shows in-flight only, not failure history — nothing persists a failed attempt once its toast dismisses (confirmed zero `error`/`status` fields anywhere in the schema); real failure/retry history belongs with roadmap #4's `GenerationEvent` once that lands, not a parallel table built here.
- [x] 2.3 Failure states as Problem → Explanation → Action (`components/generation-error.tsx`), inline near every generate button, naming the provider/model that actually ran (routes now echo `modelId`/`provider` in error JSON, already resolved server-side but previously dropped) — session-local state, not persisted, same reasoning as 2.2's gap note.
- [ ] 2.4 Pre-generation cost/scope estimate — **out of scope, not attempted**. Roadmap #4 (`GenerationEvent` + `usage.cost` capture) is marked "MUST BUILD NOW — highest leverage" and owned by `ai-architect`/`technical-architect`; confirmed fully unbuilt (zero cost/usage capture anywhere in `openrouter.ts`). Building a parallel cost estimate here would risk duplicating that work. Revisit once #4 ships.

### Phase 3 — Structural (4–6 weeks) — 3.2–3.6 shipped 2026-09-11, 3.1 deferred

- [ ] 3.1 Scene workspace: scene rail + focused scene (list-detail), unifying SINGLE and SERIES — **deliberately deferred**, not attempted this pass. The only Phase 3 item touching `scene-manager.tsx`/`scene-video-panel.tsx`/`shot-manager.tsx`, which have a separate, still-uncommitted work stream actively landing features in them (per-pair video retake, camera-movement prompts, a per-shot model override). Per explicit user direction: sequence this last, after those files settle. Confirmed via research it's mostly an outer-layout change (SceneRow is already self-contained and reusable as the detail pane; no SINGLE/SERIES divergence exists to unify) — revisit once the parallel work lands.
- [x] 3.2 Promoted assembly to an explicit 4-stage sequence (Silent → Cue Plan → Final → Download/Share) — numbered "Step N of 4" Card headers; `VideoAssemblyPanel` now hard-gates on `hasSelectedSilentVideo` (previously ungated) and was split into two Cards (Final Assembly, Download & Share) so the two are visually and functionally distinct stages. Cue Plan → Final Assembly stayed a soft/numbered step, not a hard gate — confirmed "Apply All" writes directly into Scene/DialogueLine fields with no distinguishable "cue plan applied" flag to gate on (manually-written fields are indistinguishable from cue-plan-applied ones by design).
- [x] 3.3 Per-project model settings (new `ProjectModelDefault` table + `/projects/[id]/model-settings` page) — 3-tier resolution (per-action override → per-project default → global `isDefault`) generalizing the pattern `Shot.videoModelId`'s override already established; wired into `ModelSelect` via an optional `projectId` prop (existing call sites unaffected) and applied to `scene-manager.tsx`'s top-level selects. Seedance's structured prompt builder (`PromptBuilderFields`/`assemblePrompt`/failure-mode table) generalized into a shared `video-prompt-builder.tsx` module and added to `scene-video-panel.tsx` as a toggle-able alternate input mode; `seedance-studio.tsx` now imports the same module instead of its own copy (dedup). Per user's call: the Seedance route itself was **not** retired.
- [x] 3.4 Responsive pass — fixed the two always-present, non-degrading chrome elements (`site-header.tsx`: admin links/email collapse behind `sm:`; `project-nav.tsx`: horizontal scroll instead of overflow/wrap). Per-panel controls already degraded acceptably via existing `flex-wrap`, confirmed by direct review — no changes made there to avoid touching the contested hot files further than necessary.
- [x] 3.5 Voice picker — new `VoicePicker` component (search + play-preview + pick), backing a real ElevenLabs `GET /v1/voices` call (response shape verified live, not guessed) and a hardcoded, docs-verified Sarvam bulbul:v3 speaker roster (37 names, Sarvam has no list-speakers endpoint). Wired into `character-detail-form.tsx` and the narrator-voice field in `scene-manager.tsx`; `voiceName` stays free text, no schema change.
- [x] 3.6 `ThemeProvider` + toggle — wired `next-themes` (was an unused dependency) into `layout.tsx`, added `ThemeToggle` in the header. CSS tokens were already complete, confirmed before starting — this was purely a wiring task.
